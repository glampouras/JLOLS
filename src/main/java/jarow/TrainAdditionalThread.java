            ...
            for (Instance instance : instances) {
                ...
                if (Double.compare(instance.getCosts().get(prediction.getLabel()), 0) > 0) {
                    ...
                    if (adapt) {
                        ...
                        for (String s : classifier.getCurrentVarianceVectors().get(prediction.getLabel()).keySet()) {
                            if (classifier.getCurrentVarianceVectors().get(prediction.getLabel()).get(s) < 0) {
                        ...
                        for (String s : classifier.getCurrentVarianceVectors().get(minCorrectLabel).keySet()) {
                            if (classifier.getCurrentVarianceVectors().get(minCorrectLabel).get(s) < 0) {
                                System.out.println(classifier.getCurrentVarianceVectors().get(minCorrectLabel));
                            }
                        }
                        for (String s : classifier.getCurrentGlobalVarianceVectors().keySet()) {
                            if (classifier.getCurrentGlobalVarianceVectors().get(s) < 0) {
